...
public class TestReclamacaoController {
	...
	public void testFindAll() throws DaoException {
        ...
        Assert.assertEquals(lista.get(0).getReclamacao().getCodigo(), findAll.getBody().get(0).getReclamacao().getCodigo());
	}
	...
	public void testFindBy() throws DaoException {
		...
		ResponseEntity<ReclamacaoPayload> findBy = controller.findBy(123);

		Assert.assertNotNull(findBy);
        Assert.assertEquals(200, findBy.getStatusCode().value());
        Assert.assertEquals(getMock().getCodigo(), findBy.getBody().getReclamacao().getCodigo());
	}
